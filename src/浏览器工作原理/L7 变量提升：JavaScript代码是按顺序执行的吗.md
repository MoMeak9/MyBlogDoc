...
- 第3行，由于这行是经过var声明的，因此JavaScript引擎将在**环境对象中创建一个名为myname的属性，并使用undefined对其初始化**；
...
- 在编译阶段，**变量和函数会被存放到变量环境**中，变量的默认值会被设置为undefined；在代码执行阶段，JavaScript引擎会从变量环境中去查找自定义的变量和函数。
