...
- 在函数的作用域**内部，通过let声明的变量并没有被存放到词法环境中。**
...
其实，**在词法环境内部，维护了一个<u>小型栈结构</u>，栈底是函数最外层的变量，进入一个作用域块后，就会把该作用域块内部的变量压到栈顶；当作用域执行完成之后，该作用域的信息就会从栈顶弹出，这就是词法环境的结构**。需要注意下，我这里所讲的变量是指通过let或者const声明的变量。
