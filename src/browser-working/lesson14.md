...
![编译器和解释器“翻译”代码](https://mc-web-1259409954.cos.ap-guangzhou.myqcloud.com/MyImages/202204081238798.png)

从图中你可以看出这二者的执行流程，大致可阐述为如下：

1. 在编译型语言的编译过程中，编译器首先会依次对源代码进行**词法分析、语法分析，生成抽象语法树（AST），然后是优化代码，最后再生成处理器能够理解的机器码**。如果编译成功，将会生成一个可执行的文件。但如果编译过程发生了语法或者其他的错误，那么编译器就会抛出异常，最后的二进制文件也不会生成成功
2. 在解释型语言的解释过程中，同样解释器也会对源代码进行**词法分析、语法分析，并生成抽象语法树（AST）**，不过它会再基于抽象语法树生成字节码，最后再根据字节码来执行程序、输出结果。
...
![V8 执行一段代码流程图](https://mc-web-1259409954.cos.ap-guangzhou.myqcloud.com/MyImages/202204081249072.png)
...
![抽象语法树（AST）结构](https://mc-web-1259409954.cos.ap-guangzhou.myqcloud.com/MyImages/202204081249699.png)
...
![分解 token 示意图](https://mc-web-1259409954.cos.ap-guangzhou.myqcloud.com/MyImages/202204081249878.png)
...
![字节码和机器码占用空间对比](https://mc-web-1259409954.cos.ap-guangzhou.myqcloud.com/MyImages/202204081250668.png)
...
![即时编译（JIT）技术](https://mc-web-1259409954.cos.ap-guangzhou.myqcloud.com/MyImages/202204081250905.png)
