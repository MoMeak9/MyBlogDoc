...
- **BigInt** 是一种数字类型的数据，它可以表示任意精度格式的**整数**，使用 BigInt 可以安全地存储和操作大整数，即使这个数已经超出了 Number 能够表示的安全整数范围。
...
- <u>**栈区内存**</u>：**由编译器自动分配释放，存放函数的参数值，局部变量的值等**。其操作方式类似于数据结构中的栈。
- **<u>堆区内存</u>：**<u>一般由开发着分配释放，若开发者不释放，程序结束时可能由垃圾回收机制回收。</u>
